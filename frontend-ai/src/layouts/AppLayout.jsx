...
const NAV = [
  { to: '/chat',      id: 'ai-btn',       label: 'AI Assistant', icon: <BuildingMark size={22} /> },
...
export default function AppLayout() {
  ...
  // Default collapsed on first load (chat gets full focus, Claude-style); the
  // toggle still reveals it, and the user's choice is then remembered.
  const [insightsCollapsed, setInsightsCollapsed] = useState(() => {
    const v = localStorage.getItem('insights_collapsed')
  ...
  // Draggable FAB — horizontal only. fabX = the button's centre X (px); null = centred.
  const [fabX, setFabX] = useState(() => {
    const v = localStorage.getItem('fab_x')
    return v ? Number(v) : null
  })
  const fabDrag = useRef({ active: false, moved: false, startX: 0 })

  const onFabPointerDown = (e) => {
    fabDrag.current = { active: true, moved: false, startX: e.clientX }
    try { e.currentTarget.setPointerCapture(e.pointerId) } catch {}
  }
  const onFabPointerMove = (e) => {
    if (!fabDrag.current.active) return
    if (Math.abs(e.clientX - fabDrag.current.startX) > 5) fabDrag.current.moved = true
    const half = 34
    const x = Math.min(window.innerWidth - half, Math.max(half, e.clientX))
    fabDrag.current.lastX = x
    setFabX(x)
  }
  const onFabPointerUp = () => {
    if (fabDrag.current.moved && fabDrag.current.lastX != null) {
      localStorage.setItem('fab_x', String(fabDrag.current.lastX))
    }
    fabDrag.current.active = false
  }
  const onFabClick = () => {
    if (fabDrag.current.moved) { fabDrag.current.moved = false; return }  // it was a drag
  ...
  useEffect(() => {
    ...
    // Suppress layout transitions during the route switch so the grid doesn't
    // animate its columns (which reflows the heavy chat every frame → jank).
    // The switch snaps cleanly, then transitions are restored for toggles.
    setIsMounting(true);
    ...
    <div className="app">
      ...
      {location.pathname !== '/chat' && (
        <button
          ...
          style={{ '--fab-x': fabX != null ? `${fabX}px` : '50%', touchAction: 'none' }}
          title="Ask BizAssist"
          aria-label="Ask BizAssist (drag to move)"
        >
          <svg className="fab-waves" width="64" height="64" viewBox="0 0 64 64" aria-hidden="true">
            <path className="fab-wave fw1" d="M0 33 q 8 -5 16 0 t 16 0 t 16 0 t 16 0 t 16 0 t 16 0 t 16 0 t 16 0 V64 H0 Z" />
            <path className="fab-wave fw2" d="M0 39 q 8 -4 16 0 t 16 0 t 16 0 t 16 0 t 16 0 t 16 0 t 16 0 t 16 0 V64 H0 Z" />
            <path className="fab-wave fw3" d="M0 45 q 8 -3 16 0 t 16 0 t 16 0 t 16 0 t 16 0 t 16 0 t 16 0 t 16 0 V64 H0 Z" />
          </svg>
          <span className="fab-logo"><BuildingMark size={26} /></span>
        </button>
      )}
    </div>
  )
