...
3. **Re-number on live collision** — `create_sale_invoice(renumber_on_conflict=…)`: when a sale is committed and its number is already taken (a concurrent same-series bill grabbed it; true retries are absorbed by the `X-Client-Request-Id` wall), the server allocates the **next free number in the same series** (`_free_invoice_number`, preserves the exact prefix incl. `LCL-`) instead of returning/merging the existing row → no silent lost sale, no gaps. Routes pass `renumber_on_conflict=guard.active`; the frontend prints the **server-returned** number (`usePaymentFlow`). Tests: `test_billing.py::{test_renumber_on_conflict_creates_distinct_bill, test_renumber_preserves_lcl_series_prefix, test_default_still_idempotent_on_duplicate_number}`. *(Full server-authoritative-at-commit numbering is the planned end state; this is the safety net.)*
