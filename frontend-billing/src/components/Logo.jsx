 ...
 * components/Logo.jsx
 * ====================
 * Billing-side port of the BizAssist brand mark from frontend-ai.
 * Uses accent token `var(--accent)` (terracotta) to match the billing palette.
 *
 * Exports:
 *   BuildingMark   — static outlined skyline + windows + doors + star
 *   NavMark        — compact 24-grid icon for sidebar/nav
 *   Logo           — mark + "BizAssist" wordmark
 *   SkylineLoader  — animated rising-towers loader (full or inline)
 */
...
// ─── Static detailed building mark ───────────────────────────────────────────
export function BuildingMark({ size = 28, accent = 'var(--accent)', strokeWidth = 1.8, style }) {
  return (
    <svg
      width={size}
      height={size}
...
export function NavMark({ size = 18 }) {
  return (
    ...
    >
      ...
      <path
        d="M15.8 0.5 L16.6 2.4 L18.5 3 L16.6 3.6 L15.8 5.5 L15 3.6 L13.1 3 L15 2.4 Z"
        fill="var(--accent)"
        stroke="none"
      />
    </svg>
    ...
    >
      ...
      {wordmark && (
        <span style={{
          ...
          Biz<span style={{ color: 'var(--accent)' }}>Assist</span>
        </span>
      )}
    </span>
  )
...
// ─── Animated rising-tower loader (exact frontend-ai port) ────────────────────
function Tower({ x, y, w, h, windows, door }) {
  return (
    <>
      ...
      <g fill="currentColor">
        {windows.map(([wx, wy], i) => (
          <rect key={i} x={wx} y={wy} width="2.1" height="2.1" />
        ))}
      </g>
      <rect x={door[0]} y={door[1]} width={door[2]} height={door[3]}
...
export function SkylineLoader({ size = 92 }) {
  // Unique clip IDs so multiple loaders can render simultaneously
    ...
    <svg width={size} height={size} viewBox="0 0 64 64" fill="none" aria-hidden="true">
      ...
      <g clipPath={`url(#${c1})`}>
        <g className="ba-rise" style={{ '--d': '18px', animationDelay: '0s' }}>
          <Tower x="8"  y="36" w="12" h="18"
            windows={[[10.6, 39], [15.1, 39]]}
            door={[11.4, 46, 5.2, 8]} />
        </g>
      </g>
      <g clipPath={`url(#${c2})`}>
        <g className="ba-rise" style={{ '--d': '28px', animationDelay: '.18s' }}>
          <Tower x="26" y="26" w="12" h="28"
            windows={[[28.5, 29], [33.2, 29], [28.5, 34], [33.2, 34]]}
            door={[29.4, 43, 5.2, 11]} />
        </g>
      </g>
      <g clipPath={`url(#${c3})`}>
        <g className="ba-rise" style={{ '--d': '38px', animationDelay: '.36s' }}>
          <Tower x="44" y="16" w="12" h="38"
            windows={[[46.5, 23], [51.2, 23], [46.5, 28], [51.2, 28], [46.5, 33], [51.2, 33]]}
            door={[47.4, 40, 5.2, 14]} />
        </g>
      </g>
      <path className="ba-rise-star" d={STAR} fill="var(--accent)" />
    </svg>
  )
}

// Alias for small inline usage (chat typing indicator, etc.)
