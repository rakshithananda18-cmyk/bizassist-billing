...
const LABELS = {
  'cloud-to-local': {
    ...
    steps: ['Reading data from cloud', 'Merging into local', 'Verifying'],
  ...
  'local-to-cloud': {
    ...
    steps: ['Reading data from this device', 'Merging into cloud', 'Verifying'],
...
export default function BackupModal({ token, direction = 'cloud-to-local', onComplete, onError }) {
  ...
  async function run() {
    ...
    try {
      ...
      setSummary({ total: imData?.total ?? 0 })
      ...
      logger.info(`[SYNC] ${direction} complete: ${imData?.total ?? 0} records merged`)
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: 'var(--bg-2, #1a1a1a)', border: '1px solid var(--border, rgba(255,255,255,0.12))', borderRadius: 16, padding: '30px 34px', width: '100%', maxWidth: 480, boxShadow: '0 24px 80px rgba(0,0,0,0.5)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 18 }}>
          {STEPS.map((step, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, opacity: statuses[i] === 'pending' ? 0.45 : 1 }}>
              ...
              <div style={{ fontSize: '0.84rem', fontWeight: statuses[i] === 'active' ? 700 : 500, color: statuses[i] === 'error' ? '#ef4444' : statuses[i] === 'done' ? '#22c55e' : 'var(--text-primary)' }}>{step}</div>
            </div>
          ))}
        </div>

        {done && summary && (
          <div style={{ background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.25)', borderRadius: 8, padding: '10px 14px', fontSize: '0.8rem', color: '#22c55e', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 6 }}>
            <ShieldIcon size={14} strokeWidth={2} /><span>{summary.total} records merged (newer wins; nothing overwritten).</span>
          </div>
        )}
        {failed && (
        ...
        {(done || failed) && (
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
            {failed && (
              <button onClick={() => { setStatuses(STEPS.map((_, i) => (i === 0 ? 'active' : 'pending'))); setErrorMsg(null); setProgress(0); run() }}
