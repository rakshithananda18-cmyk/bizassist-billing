...
- 🟡 **R7b Slice 3a — client offline sync core (2026-06-22):** pure node-tested modules in `frontend-billing/src/sync/` — `uuid` (stable client-request id), `stores` (memory + IndexedDB, injectable), `outbox` (FIFO queue: success-drop / transient-stop / 4xx-dead-letter / retry-cap), `cursor` (monotonic pull cursor), `syncManager` (mutate/flush/pull seam over `api/client`, flush-on-`online`). Replays against the Slice-1 wall → exactly-once. Tests `sync_outbox.test.js` + `sync_manager.test.js`; logic sim-verified in node. (§10.3 R7b Slice 3a.)
- 🟡 **R7b Slice 3c — true offline POS save (2026-06-22):** offline/network-fail → bill **queued in IndexedDB outbox** (not lost), thermal print from local state, local reset; flush + refetch + "N unsynced" badge on reconnect. **Critical fix:** offline invoice numbering folds queued bills into the allocator (`mergePending`/`pendingInvoiceRows`) so two offline bills can't collide on one `invoice_no` (which the inner wall would drop). Exactly-once via stable id + unique invoice_no. Test `sync_pending_invoices.test.js`; sim-verified. **Needs user `npm run dev` offline test** (2 bills offline → distinct numbers → reconnect → exactly-once). (§10.3 R7b Slice 3c.)
- 🟡 **R7b Slice 3b — POS save exactly-once (2026-06-22):** **fixed a Slice-1 gap** — the POS "Save Bill" posts to `POST /invoices` (not the guarded `/sales`); added `replay_guard` there + test. Frontend `usePaymentFlow` attaches a **stable `X-Client-Request-Id` per bill** (retry-after-network reuses it → exactly-once; success/4xx clears it). CORS `allow_headers` += `X-Client-Request-Id` (restart backend). (§10.3 R7b Slice 3b.)
- 🟡 **R7b Slice 2 — offline delta PULL `GET /sync/pull` (2026-06-22):** read-only `core/api/sync.py` returns tenant-scoped changes since a **per-entity id cursor** (`?since={"invoice":120,…}` → `{changes,cursor,has_more,limit}`). **Derived from append-only source tables (no table/migration/money-path touch):** `id > cursor`, ordered, `limit+1` sentinel → exactly-once, gap-free, can't drift. Covers money entities (invoice/payment/purchase/stock), header-level; catalog-edit sync deferred. `tests/test_sync_pull.py`; cursor/paging/isolation sim-verified. *User runs `pytest`.* (§10.3 R7b Slice 2.)
- 🟡 **R7b Slice 1 — generalized exactly-once replay wall (2026-06-22):** new `idempotency_keys` table (migration `f4a1c7e9b2d6`, new head; additive/idempotent/`create_all`-registered → **no test-DB rebuild**) + `core/sync/idempotency.py` `ReplayGuard`/`replay_guard` reading the **`X-Client-Request-Id`** header, wired into `POST /sales`, `POST /payments`, `POST /purchases/confirm`. Replays the stored response on retry/outbox-flush instead of double-posting; inert without the header (backward-compat); tenant-scoped; race-safe via `UNIQUE(business_id, client_request_id)`. The OUTER wall over the existing per-command (INNER) idempotency. Logger `bizassist.sync` `[SYNC]`. `tests/test_sync_idempotency.py` (same-key→one row + replay, no-header control, different-keys, **cross-tenant not replayed**, payment exactly-once); replay/store/race sim-verified in-sandbox. *Still deferred:* client outbox + `GET /sync/pull` delta cursor + Postgres RLS (Slices 2–4). *User runs `alembic upgrade head` + `pytest`.* (§10.3 R7b Slice 1.)
...
| Offline / sync (D5) | 2 | **4** | **R7b Slices 1–3 landed (server + client):** (1) exactly-once **PUSH replay wall** (`X-Client-Request-Id`→`idempotency_keys`, on `/sales`+`/payments`+`/purchases/confirm`+the real POS `/invoices` route); (2) **delta PULL** `GET /sync/pull` (per-entity id cursor, can't drift); (3) **client IndexedDB outbox** + offline detection — POS "Save Bill" now queues when offline, prints thermal locally, allocates collision-free offline invoice numbers, shows an "unsynced" badge, and flushes exactly-once on reconnect. The offline experience now EXISTS on-device. Held below 5 because it's **not yet user-verified live** (`npm run dev` offline scenario pending) and **Postgres RLS** (the DB-enforced second tenancy wall) is still deferred (Slice 4). |
...
- **R7b Slice 1 — generalized exactly-once replay wall · [🟡 DONE 2026-06-22]** — the *server* foundation the offline outbox needs. New `idempotency_keys` table (`core/models.py` → migration `f4a1c7e9b2d6`, new head; additive, `has_table`-guarded, registers via `create_all` so **no test-DB rebuild**): `UNIQUE(business_id, client_request_id)` storing the response body. New `core/sync/idempotency.py` `ReplayGuard` + `replay_guard` FastAPI dependency reads the **`X-Client-Request-Id`** header — on a hit it replays the stored response verbatim, on a miss it does the work then stores it; **inert when the header is absent** (full backward-compat), **tenant-scoped**, and **race-safe** (concurrent duplicate INSERT hits the UNIQUE constraint → rolls back its own row and returns the winner's stored response). Wired into the three money-mutating routes: `POST /sales`, `POST /payments`, `POST /purchases/confirm`. This is the **OUTER wall**; the existing per-command idempotency (sale `invoice_no`, payment `idempotency_key`, `post_entry` source-key, B2B order-sync guard) stays the **INNER wall** — together they guarantee no double-post even under concurrency *and* a consistent reply on replay. Logger `bizassist.sync`, tag `[SYNC]`. Tests: `tests/test_sync_idempotency.py` (same key → one invoice + replayed body; no-header control → distinct rows; different keys → distinct; **same key cross-tenant → not replayed**; payment replay → one receipt). Pure replay/store/race logic sim-verified in-sandbox (stdlib sqlite, synthetic data). *Still deferred (Slices 2–4):* client IndexedDB outbox + offline detection + flush-on-reconnect, `GET /sync/pull?since=` delta cursor, Postgres RLS (the second wall).
- **R7b Slice 2 — offline delta PULL (`GET /sync/pull`) · [🟡 DONE 2026-06-22]** — the PULL half (Slice 1 was the PUSH/replay half). New read-only endpoint `core/api/sync.py` (wired in `core/api/__init__.py`) returns tenant-scoped changes since a **per-entity autoincrement-id cursor** — `?since={"invoice":120,"payment":45,"purchase":7,"stock":300}` → `{changes, cursor, has_more, limit}`. **Derived feed, no new table, no migration, no money-path touch:** it reads the append-only source tables directly (`id > cursor`, `ORDER BY id ASC`, `limit+1` sentinel for `has_more`), so delivery is **exactly-once + gap-free** and the feed **can never drift** from the truth (no separate log to maintain). Cursors are per-tenant high-water marks; `business_id == caller` on every query. Capped per entity (default 500, ≤2000) for backpressure. **Scope:** append-only money entities (invoice/payment/purchase/stock), header-level fields (no N+1 line-item hydration — client uses the existing detail endpoints); in-place catalog edits need an `updated_at` watermark → later slice. Logger `bizassist.sync` `[SYNC]`. Tests: `tests/test_sync_pull.py` (backfill→re-pull-empty/stable-cursor, only-new-past-cursor, `has_more` paged drain exactly-once, **tenant isolation + per-tenant cursor independence**, garbage-cursor → safe backfill). Cursor/paging/isolation logic sim-verified in-sandbox (stdlib sqlite). *Still deferred (Slices 3–4):* client IndexedDB outbox + offline detection + flush-on-reconnect, then Postgres RLS.
- **R7b Slice 3a — client offline sync core · [🟡 DONE 2026-06-22, needs `npm run dev` wiring]** — the on-device half, built as PURE node-testable modules in `frontend-billing/src/sync/` so the logic is verified without a browser: `uuid.js` (stable `X-Client-Request-Id` per user intent), `stores.js` (injectable store: in-memory for tests + durable **IndexedDB** for the app, identical async interface), `outbox.js` (FIFO write queue — **success drops**, **transient network/5xx STOPS** to preserve order, **4xx DEAD-LETTERS and continues** so a poison op can't wedge the queue, **retry cap** dead-letters a stuck transient), `cursor.js` (monotonic delta-pull cursor merge), and `syncManager.js` (the seam: `mutate` = retry-safe money mutation that sends online with the stable id / queues when offline / re-queues on transient failure / throws 4xx to the caller; `flushOutbox` on reconnect; `pull` advances the stored cursor; `start()` flushes on the `online` event). All replay against the backend's Slice-1 wall so a flush is exactly-once. Tests: `src/__tests__/sync_outbox.test.js` + `sync_manager.test.js` (FIFO/transient-stop/dead-letter/retry-cap/cursor-monotonic + mutate offline/sent/transient/4xx + flush-replays-id + pull-cursor-advance). Logic sim-verified in-sandbox (node, all 6 checks).
- **R7b Slice 3b — POS save exactly-once (backend gap fix + header wiring) · [🟡 DONE 2026-06-22, needs `npm run dev`]** — **Found and fixed a Slice-1 gap:** the POS counter "Save Bill" actually posts to **`POST /invoices`** (`create_sale_invoice_frontend`, which `authFetch` maps `/billing/invoices`→`/invoices`), NOT the `/sales` route Slice 1 guarded. Added the `replay_guard` to `POST /invoices` so the route the offline outbox will flush is covered by the outer wall (the inner `invoice_no` wall already prevented a double-post; this adds the consistent-replayed-response guarantee). Test added to `tests/test_sync_idempotency.py` (`test_pos_invoices_route_replays_one_invoice`). **Frontend:** `usePaymentFlow` now attaches a **stable `X-Client-Request-Id` per bill** (keyed by the tab's invoice number via a `useRef` map): a retry after an ambiguous NETWORK failure reuses the id → exactly-once; a clean success or a 4xx rejection clears it → next save is a fresh intent. Small, money-path-safe change (no flow reshape). **CORS:** the custom header makes the POST a non-simple request → browser preflight; `main_groq.py` `allow_headers` extended to include `X-Client-Request-Id` (was `Authorization, Content-Type` only — preflight was 400-rejected until added). *Requires a backend restart.*
- **R7b Slice 3c — true offline POS save · [🟡 DONE 2026-06-22, needs live `npm run dev` offline test]** — the actual offline experience. `usePaymentFlow` now: when `navigator.onLine` is false (or the POST throws a network/CORS error) it **queues the bill in the durable IndexedDB outbox** via `syncManager.queue` instead of erroring, prints **thermal receipts from local state** (PDF print is skipped offline with a notice), and resets the tab locally (no server refetch). A `Session expired` error is still surfaced, not queued. **Critical correctness fix — offline invoice numbering:** the POS allocates the next number from the SERVER list, which is stale offline; without care, two offline bills get the SAME `invoice_no` and the backend's inner wall would silently DROP the second. `Sales.jsx` now mirrors the outbox's pending bills into a ref and folds their numbers into the allocator (`mergePending` + `sync/pendingInvoices.pendingInvoiceRows`) at every allocation site (`load`, `handleNewBill`, last-tab `closeTab`), so each offline bill gets a unique number. On reconnect (the `online` event, and on mount) `Sales.jsx` flushes the outbox, refetches the server list, and refreshes an **"N bills saved offline — will sync" badge**. Replay is exactly-once because each queued op carries its stable `X-Client-Request-Id` (Slice 1 wall) + its unique `invoice_no` (inner wall). Tests: `src/__tests__/sync_pending_invoices.test.js` (number extraction/merge → next number skips queued bills); number-merge + allocator logic sim-verified in node. **Needs USER `npm test` + `npm run dev` with DevTools offline:** save 2+ bills offline (distinct numbers, badge increments, thermal prints), then go online and confirm they sync exactly once with no duplicate/dropped bill. **Then Postgres RLS (Slice 4) closes R7b.**
