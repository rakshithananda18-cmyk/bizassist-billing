...
**✅ LLM-router decision fix pack (landed this session — from the live-trace review).**
The new LLM router (`services/llm_router.py`, `LLM_ROUTER=on`) *decided* well but the pipeline didn't *use* everything it decided. All fixes are "honor the decision object," not new layers. Extracted `_resolve_llm_decision()` (pure, unit-tested) so the guards are testable.
- **B3 (worst trust bug) — `invoice_detail` returned nothing → LLM hallucinated a different invoice/amount.** Root cause: `INVOICE_ID_RE` required a prefix (`XXX-INV-####`) but the real data is bare `INV-0007`, so the handler silently failed. Fixed the regex (optional prefix) in both `handlers/invoices.py` and `ai_router._INVOICE_ID_RE`; `_invoice_detail` now accepts an explicit `invoice_id`; `direct_query_handler.handle` passes `decision.entities` through (`params`).
- **B1 — confidence floor.** Below `LLM_ROUTER_CONF_FLOOR` (0.6) the LLM decision is dropped and the legacy decision stands. Kills "Do you know rahul traders" (`chat conf=0.00` → was "I'm not aware…") and "Draft a follow-up to this customer" (`act conf=0.00`, context lost).
- **B2 — honor extracted entities.** A named customer + a non-customer-scoped answer/chat intent now reroutes to `client_summary` ("How much does Nilgiris Fresh owe me?" no longer returns the global top-debtors table); never drops to chat when a customer is named; per-customer rerouting also fixes the `cache_disc='top_debtors'` collision (Annapurna was served Nilgiris's cached table).
- **B4 — action preview honors the named target.** `send_payment_reminders` preview/execute now scopes to a single `customer`/`invoice_id` hint (fuzzy-resolved to a real overdue customer), so "Follow up with Sri Venkateswara about INV-0007" previews 1 customer, not all 20.
- **B5 — advise prompt bans hypotheticals.** Hard rule: never invent example names/amounts (killed "if 'Reliance Retail' has ₹50,000…"); collection/DSO advice is now also grounded on the real top-debtors table.
- **B6 — fast-path skip.** When legacy matched an exact dashboard template (`overdue_range_detail`/`revenue_month_detail`) or an override applied, skip the LLM router's 1-2s call.
- **B7 — log hygiene.** REQ/CACHE lines print the effective (query-keyed) `cache_disc` for AI_ADVISE; the `[AI_COMPLEX]` line says `agent-loop` vs `LangGraph agents` per `AGENT_MODE`.
- Covered by `test_router_fixpack.py` (B1/B2 via the pure helper; B3/B4 against a seeded DB; B5 prompt assertion) + the existing `test_llm_router.py`.

**✅ Two trust/UX polish items (landed after the live re-test).**
- **Honest quota message.** A Groq daily-token (TPD) 429 in the agent loop now shows *"The daily AI analysis limit has been reached … quick answers still work … resets within an hour"* instead of the generic "advisor hit an error." `agent_loop._friendly_error()` detects 429/rate-limit/quota; both stream and non-stream paths use it (non-stream now wrapped in try/except too). The deep-analysis 429s in the live trace were this cap (free-tier 100k/day on the 70B), not a code regression.
- **No fake "Done."** The AI_SIMPLE prompt now forbids the 8B from ever claiming it performed an action ("Done."/"Sent."/"I've escalated…") — it can't execute anything, so bare commands like "Send reminders" must draft + point to the action button. Covered by new cases in `test_agent_loop.py`.
