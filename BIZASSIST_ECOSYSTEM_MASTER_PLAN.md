...
**Phase 1 — Billing painkiller** · *status: 🟡 mostly built, finishing depth*
...
**Phase 6 — Deep moat** · *status: ⬜* — reputation/trust score, signed + hash-chained shared ledger, e-invoice IRN API, Redis at scale.

**Immediate next (P0/P1 from the Vyapar benchmark):** finish `Sales.jsx` decomposition → verify print/share/UPI-QR + returns → staff RBAC → then accounting depth (balance sheet/day book) and compliance (e-invoice IRN, e-way API). Protect the USP (connections + sync + AI) once billing depth is solid.
...
5. **The books are only audit-grade going forward.** The posted journal starts now; historical data is still *reconstructed*. ✅ **Period close/lock now shipped (R2)** — a closed period rejects any new/changed posting (422), enforced at the single `post_entry` choke point, so the books can no longer be altered upstream once locked. **Remaining (R2b):** opening-balance carry-forward so reports don't re-derive from genesis every time.
...
**R2b — Opening-balance carry-forward · M · backend core**
- *Approach:* `opening_balances` (business_id, account, as_of_date, balance) snapshot written at period close; reports read the latest snapshot ≤ window-start instead of replaying from genesis; ties BS/TB across the boundary. *Tests:* carry-forward ties balances across a locked boundary; closing of period N == opening of period N+1.
...
**R5 — Decompose god-components · M · medium risk · frontend**
- *Problem (§10.2 #4):* `Sales.jsx` ~2,800 lines; `Reports.jsx` is a 7-branch render ternary.
- *Approach:* continue the started extraction — `usePaymentFlow` is done; next lift `<PaymentPanel>` (the ref-heavy shell), then split `Reports.jsx` into one component per report behind a small registry/map (kills the ternary). Each piece gets a render test.
- *Tests:* render/interaction tests per extracted component; money path unchanged (covered by R4/invoiceMath). *Risk:* the payment panel carries ~25 props + refs — extract incrementally, not blind.
