...
# =============================================================================
# Superseded by routes/data_transfer.py, which main_groq.py mounts. This module
# declares /api/migrate/* — NOT SERVED. The live one declares /api/data-transfer/*.
#
# TWO HAZARDS live in the code below. Do not port it verbatim:
#   1. `_upsert_users` matches staff on the per-database `username`, so it never
#      matches across a transfer and inserts a duplicate every run. That is what
#      put 24 cashier logins on a business with 2 tills.
#   2. Its owner update_fields include `public_id`, so an import payload could
#      OVERWRITE the destination business's BizID — the tenant identity spine.
#      data_transfer.py excludes it.
#   3. On SQLite it uses INSERT OR REPLACE INTO users, which resolves on the
#      PRIMARY KEY: a source row whose id collides with an unrelated local user
#      replaces that user outright, password hash included.
#
# One thing here was BETTER and has already been ported out:
# `_free_invoice_number_on_import` (M-3 x §9.3b) existed only in this file, so
# the live import path was silently DROPPING bills whose number was already
# taken. Ported to data_transfer.py on 2026-07-31.
#
# Full analysis: docs/CLEANUP_PLAN_2026-07-31.md §1
# =============================================================================

# """
# routes/migrate.py — SUPERSEDED, NOT MOUNTED. DO NOT REVIVE AS-IS.
# =================================================================
#
# `main_groq.py` imports and mounts `routes/data_transfer.py`, not this file.
# Nothing imports this module. It is an older copy of the same endpoints, left in
# the tree, and its `_upsert_users` still carries the defect that
# `data_transfer.py` has now fixed:
#
#   * it upserts staff **by `username`** — the INTERNAL, globally-unique name that
#     each database derives differently on purpose (`counter_1__7` locally,
#     `counter_1_c7` on the cloud). Across a transfer it therefore never matches,
#     and every run inserts a second row for every staff member. That is what put
#     24 cashier logins on a business that has 2.
#   * on SQLite it does `INSERT OR REPLACE INTO users`, which resolves on the
#     PRIMARY KEY — so a source row whose `id` happens to match an unrelated local
#     user REPLACES that user outright, including their password hash and role.
#
# If these endpoints are ever needed again, port `data_transfer.py`'s version
# rather than mounting this one: match staff on `staff_login_name` scoped to the
# destination owner (the identity that means the same thing in both databases —
# see core/identity.py), and never resolve an upsert on a per-database integer id.
#
# STATUS: deprecated, retained for a later cleanup pass. It is not deleted yet by
# request. Importing it raises a DeprecationWarning and logs at WARNING, and both
# defects are marked inline at the exact lines (search "DEPRECATED — DEFECT").
#
# Note the export format documented below is duplicated verbatim in
# `data_transfer.py`, so nothing is lost whenever this file does go.
#
# ────────────────────────────────────────────────────────────────────────────────
...
#
# # ── DEPRECATED MODULE ────────────────────────────────────────────────────────
# # Superseded by routes/data_transfer.py, which is what main_groq.py mounts.
# # Kept only until a scheduled cleanup; nothing should import it. Importing it is
# # made noisy on purpose, because the two defects below are silent by nature and
# # the header is easy to scroll past.
# __deprecated__ = True
# __superseded_by__ = "routes.data_transfer"
#
# import warnings as _warnings
#
# _warnings.warn(
#     "routes.migrate is DEPRECATED and unmounted — use routes.data_transfer. "
#     "Its _upsert_users matches staff on the per-database `username` (duplicates "
#     "every staff row on each transfer) and its SQLite path uses "
#     "INSERT OR REPLACE INTO users, which resolves on the PRIMARY KEY and can "
#     "overwrite an unrelated local user, password hash included.",
#     DeprecationWarning,
#     stacklevel=2,
# )
# logger.warning(
#     "[DEPRECATED] routes.migrate was imported. It is superseded by "
#     "routes.data_transfer and MUST NOT be mounted — see the module docstring."
# )
#
#
# # ---------------------------------------------------------------------------
...
#             # ⚠ DEPRECATED — DEFECT 1 OF 2. DO NOT COPY THIS.
#             #
#             # Upserting staff by `username` is WRONG. That is the INTERNAL,
#             # globally-unique name and each database derives it differently on
#             # purpose (`counter_1__7` locally, `counter_1_c7` on the cloud), so
#             # it NEVER matches across a transfer and every run inserts a second
#             # row for every staff member. It is what put 24 cashier logins on a
#             # business that has 2 (live database, 2026-07-31).
#             #
#             # routes/data_transfer.py matches on
#             #   (parent_business_id = dest_owner, lower(staff_login_name))
#             # — the per-business identity that means the same thing in both
#             # databases. Port that, not this. See core/identity.py.
#             # Upsert by username
...
#                 # ⚠ DEPRECATED — DEFECT 2 OF 2, AND THE DANGEROUS ONE.
#                 #
#                 # `INSERT OR REPLACE` on SQLite resolves the conflict on the
#                 # PRIMARY KEY. `col_list` still contains `id` here (unlike the
#                 # Postgres branch above, which strips it), so a source row whose
#                 # `id` collides with an UNRELATED local user deletes that user
#                 # and reinserts the source's in its place — password hash, role
#                 # and BizID included. Integer ids are per-database, so a
#                 # collision is not unlikely; it is the normal case.
#                 #
#                 # Never resolve an upsert on a per-database integer id.
...
#     # b2b_ledgers         : append-only ledger, multiple entries per pair
