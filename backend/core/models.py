...
class ConnectionCode(Base, TimestampMixin):
    """
    Single-use, expiring code generated by a seller to link a buyer.
    """
    __tablename__ = "connection_codes"
    __table_args__ = (
        Index("ix_connection_codes_seller_code", "seller_business_id", "code"),
    )
...
class SharedLedger(Base, TimestampMixin):
    """
    Append-only ledger of shared transactions between seller and buyer.
    Tracks credit, payments, and invoices.
    """
    __tablename__ = "shared_ledgers"
    __table_args__ = (
        Index("ix_shared_ledgers_seller_buyer", "seller_business_id", "buyer_business_id"),
    )
